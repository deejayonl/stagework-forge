...
const oldFunc = `const handleNodeMove = (id: string, x: number, y: number, deltaX: number, deltaY: number) => {
      setNodes(prev => {
          let newNodes = prev;
...
const newFunc = `const handleNodeMove = (id: string, x: number, y: number, deltaX: number, deltaY: number) => {
      ...
      if (selectedNodeIds.has(id) && selectedNodeIds.size > 1) {
          ...
          newNodes = nodes.map(n => {
              if (selectedNodeIds.has(n.id)) {
                  // Only apply delta if the node isn't the one being dragged (which already has absolute x,y)
                  if (n.id === id) {
                      return { ...n, x, y };
                  }
                  return { ...n, x: n.x + deltaX, y: n.y + deltaY };
          ...
          // Move just the single node
          newNodes = nodes.map(n => n.id === id ? { ...n, x, y } : n);
      }
      setNodes(newNodes);
      addToHistory(newNodes, edges);
  };`;
