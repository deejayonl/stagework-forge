...
content = content.replace(
  /containerClasses \+= \`bg-white\/90 dark:bg-hall-900\/90 shadow-2xl shadow-hall-200\/50 dark:shadow-black\/50 hover:scale-\[1\.01\] hover:shadow-indigo-500\/10 focus-within:scale-\[1\.02\] focus-within:border-indigo-500 focus-within:ring-1 focus-within:ring-indigo-500 focus-within:shadow-\[0_0_20px_-3px_rgba\(99,102,241,0\.3\)\] \$\{compact \? 'rounded-full' : 'rounded-\[32px\]'\}\`;/,
...
content = content.replace(
  /containerClasses \+= " border-indigo-500\/50 shadow-xl shadow-indigo-500\/20 ring-1 ring-indigo-500\/30 animate-pulse-border";/,
  "containerClasses += \" border-indigo-500/50 shadow-xl shadow-[0_0_15px_rgba(99,102,241,0.5)] ring-1 ring-indigo-500/30 animate-pulse\";"
);
...
// Let's also adjust the compact text area styling.
content = content.replace(
...
console.log('PromptInput.tsx updated');
