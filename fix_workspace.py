...
# Add it right before the return statement of Workspace
good_func = """
  const handleInsertSkipLink = () => {
    updateLocalHtml([], (body) => {
      if (body.querySelector('.forge-skip-link')) return;
...
# Wait, `content.replace('  return (', good_func, 1)` might match the wrong `return(`. Let's find the main return of Workspace.
