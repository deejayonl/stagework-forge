const fs = require('fs');
const content = fs.readFileSync('./src/utils/injectEditorScript.ts', 'utf8');

const oldStr = `          if (e.data.type === 'FORGE_UPDATE_STYLE') {
            const { id, property, value, breakpoint } = e.data;
...
fs.writeFileSync('./src/utils/injectEditorScript.ts', content.replace(oldStr, newStr));
