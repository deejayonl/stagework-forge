...
const ChatMessage: React.FC<ChatMessageProps> = ({ message, onNodeSelect, nodes }) => {
  ...
  if (isSystem) {
      return (
          <div className="flex justify-center my-4 animate-fade-in px-4">
              <span className="px-3 py-1.5 rounded-[1.5rem] bg-red-500/10 text-red-500 text-[10px] font-mono border border-red-500/20 shadow-sm text-center">
  ...
  return (
    <div className={`group flex w-full mb-4 animate-bounce-in hover:-translate-y-0.5 transition-transform duration-300 ease-spring ${isUser ? 'justify-end' : 'justify-start'}`}>
      <div 
        className={`
            flex flex-col 
      ...
      >
        ...
        <div 
            className={`
                ...
                ${isUser 
                    ? 'px-4 py-2.5 bg-text-primary text-bg-main rounded-[1.5rem] rounded-tr-sm shadow-md hover:shadow-lg hover:scale-[1.02] origin-bottom-right transition-all duration-300 ease-spring' // Monochrome User Bubble
                    : isToolLog
                        ? 'p-0 bg-transparent w-full'
                        : 'px-4 py-3 bg-bg-surface border border-border-subtle text-text-primary rounded-[1.5rem] rounded-tl-sm shadow-md hover:shadow-lg hover:scale-[1.02] origin-bottom-left transition-all duration-300 ease-spring'
                }
            `}
        >
            ...
            {isToolLog ? (
                 ...
                 <div className="flex flex-col gap-2 rounded-xl bg-bg-main/50 border border-border-subtle/60 p-3 font-mono text-xs shadow-sm">
                     <div className="flex items-center gap-2 pb-2 border-b border-border-subtle/50 mb-1 opacity-70">
                         ...
                         <span className="uppercase tracking-widest text-[10px] font-bold text-hall-500">System Activity</span>
            ...
            {message.attachments && message.attachments.length > 0 && (
                <div className={`mt-3 flex flex-wrap gap-2 ${isUser ? 'justify-end' : 'justify-start'}`}>
                    {message.attachments.map((att, idx) => (
                        <div 
                            key={idx} 
                            className="group/att relative rounded-[1.5rem] overflow-hidden border border-border-subtle shadow-sm w-32 h-24 bg-black/5"
                        >
                            {att.type === 'image' && (
