...
export const MarkdownRenderer: React.FC<MarkdownRendererProps> = ({ content, onNodeLinkClick, className = '', compact = false, nodes }) => {
  ...
  const renderInline = (text: string, keyPrefix: string) => {
    ...
    return parts.map((part, i) => {
      ...
      if (nodeMatch) {
        ...
        return (
          <button
            ...
            className="inline-flex items-center gap-1.5 px-1.5 py-0.5 rounded-xl bg-accent-primary/10 text-accent-primary hover:bg-accent-primary hover:text-white transition-colors text-xs font-medium align-middle border border-accent-primary/20 mx-0.5"
          >
      ...
      if (imgMatch) {
          return (
              <span key={key} className={`block rounded-2xl overflow-hidden border border-border-subtle shadow-sm bg-bg-main/50 ${compact ? 'my-2' : 'my-3'}`}>
      ...
      // 6. Code: `text`
      if (part.startsWith('`') && part.endsWith('`')) {
        return <code key={key} className="bg-black/5 dark:bg-white/10 px-1.5 py-0.5 rounded-xl text-[0.85em] font-mono border border-border-subtle/50">{part.slice(1, -1)}</code>;
      }
    ...
    switch (block.type) {
              ...
              if (isTask) {
                  ...
                  return (
                      <li key={i} className={`flex items-start gap-2 ${indentClass} ${checked ? 'opacity-60' : ''}`}>
                           <div className={`mt-[0.3em] w-3 h-3 rounded-xl border flex items-center justify-center shrink-0 ${checked ? 'bg-accent-primary border-accent-primary' : 'border-text-secondary'}`}>
      ...
      case 'table':
        try {
          ...
          return (
            <div key={index} className={`${compact ? 'my-2' : 'my-5'} overflow-x-auto rounded-2xl border border-border-subtle shadow-sm bg-bg-surface/50`}>
      ...
      case 'codeblock':
          return (
             <div key={index} className={`${compact ? 'my-2' : 'my-5'} rounded-2xl overflow-hidden border border-border-subtle bg-bg-main/50 group/code shadow-sm`}>
