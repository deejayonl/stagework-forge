...
const OmniBar: React.FC<OmniBarProps> = ({ activeAgent, onSend, isThinking, onSelectAgent, onToggleLive, isLiveMode, referencedNodes, nodeContext, onFileUpload, onRunCanvas }) => {
    ...
    useEffect(() => {
        if (isLiveMode) {
             ...
             const session = {} as any; // Stub for removed LiveSessionService
             liveSessionRef.current = session;
             
             session.onStatusChange = (status: string) => setLiveStatus(status);
             
             // Handle delegated requests from Live Agent
             session.onAgentRequest = (request: any) => {
                 // Trigger the main AI processing pipeline while keeping Live active
                 onSendRef.current(request, undefined);
             };

             session.onAudioLevel = (level: number) => {
             ...
             session.connect(nodeContextRef.current).catch((err: any) => {
    ...
    return (
        <div className="fixed md:absolute bottom-[calc(4rem+env(safe-area-inset-bottom)+1rem)] md:bottom-8 left-1/2 transform -translate-x-1/2 w-full max-w-3xl px-4 z-[60]">
            ...
            `}>
                ...
                {activeAgent === AgentId.IMAGE && !isLiveMode && (
                    <div className="flex items-center gap-3 px-4 pt-3 pb-2 border-b border-border-subtle/30 bg-bg-surface/30 backdrop-blur-sm animate-slide-up">
                        ...
                        {/* Aspect Ratio */}
                        <div className="flex items-center gap-1 bg-black/5 dark:bg-white/5 rounded-2xl p-0.5">
                             {['1:1', '3:4', '4:3', '9:16', '16:9'].map(ratio => (
                                 <button 
                                    key={ratio}
                                    onClick={() => setImageParams(p => ({ ...p, aspectRatio: ratio as any }))}
                                    className={`px-2 py-1 text-[10px] font-medium rounded-xl transition-all ${imageParams.aspectRatio === ratio ? 'bg-bg-surface text-text-primary shadow-sm' : 'text-text-secondary hover:text-text-primary'}`}
                                 >
                                    {ratio}
                                 </button>
                             ))}
                         ...
                         <div className="flex items-center gap-1 bg-black/5 dark:bg-white/5 rounded-2xl p-0.5">
                             {['1K', '2K', '4K'].map(size => (
                                 <button 
                                    ...
                                    className={`px-2 py-1 text-[10px] font-medium rounded-xl transition-all ${imageParams.size === size ? 'bg-bg-surface text-text-primary shadow-sm' : 'text-text-secondary hover:text-text-primary'}`}
                                 >
                                    {size}
                                 </button>
                             ))}
                        ...
                        <div className="flex items-center gap-1 bg-black/5 dark:bg-white/5 rounded-2xl p-0.5">
                             {['16:9', '9:16'].map(ratio => (
                                 <button 
                                    ...
                                    className={`px-2 py-1 text-[10px] font-medium rounded-xl transition-all ${videoParams.aspectRatio === ratio ? 'bg-bg-surface text-text-primary shadow-sm' : 'text-text-secondary hover:text-text-primary'}`}
                                 >
                                    {ratio}
                                 </button>
                             ))}
                         ...
                         <div className="flex items-center gap-1 bg-black/5 dark:bg-white/5 rounded-2xl p-0.5">
                             {['720p', '1080p'].map(res => (
                                 <button 
                                    ...
                                    className={`px-2 py-1 text-[10px] font-medium rounded-xl transition-all ${videoParams.resolution === res ? 'bg-bg-surface text-text-primary shadow-sm' : 'text-text-secondary hover:text-text-primary'}`}
                                 >
                                    {res}
                                 </button>
                             ))}
                ...
                <div className="flex items-end p-2 gap-2">
                    ...
                    <div className="flex items-center gap-2 shrink-0 pb-0.5">
                        {onRunCanvas && (
                            <button
                                onClick={onRunCanvas}
                                className="w-11 h-11 rounded-full flex items-center justify-center transition-all duration-300 bg-emerald-500/10 text-emerald-500 hover:bg-emerald-500/20 hover:scale-105"
                                title="Run Canvas"
                            >
