...
const COLORS = {
  idle: [100, 110, 200] as [number, number, number],
  active: [96, 165, 250] as [number, number, number],
  toolActive: [74, 222, 128] as [number, number, number],
};
...
export function OrbCanvas({
  ...
  useEffect(() => {
    ...
    function animate() {
      ...
      const gradient3 = ctx.createRadialGradient(cx, cy, 0, cx, cy, dynamicRadius * 4);
      gradient3.addColorStop(0, `rgba(${r},${g},${b},0.03)`);
      gradient3.addColorStop(1, `rgba(${r},${g},${b},0)`);
      ctx.fillStyle = gradient3;
      ctx.fillRect(0, 0, w, h);

      const gradient2 = ctx.createRadialGradient(cx, cy, 0, cx, cy, dynamicRadius * 2.2);
      gradient2.addColorStop(0, `rgba(${r},${g},${b},0.08)`);
      gradient2.addColorStop(0.5, `rgba(${r},${g},${b},0.03)`);
      ...
      gradient.addColorStop(0, `rgba(${Math.min(r + 60, 255)},${Math.min(g + 60, 255)},${Math.min(b + 60, 255)},0.25)`);
      ...
      gradient4.addColorStop(0, `rgba(255,255,255,${isActive ? 0.5 : 0.2})`);
